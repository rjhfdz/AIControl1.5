...
public class YaoMaiModelUI implements ActionListener{
		...
		dialog.addWindowListener(new WindowAdapter() {
			public void windowClosed(WindowEvent e) {
				if (runTable.getRowCount() > 0) {
					...
					Data.YaoMaiMap.put("TableData"+setNo, vectors);
				} else {
					if (radioButton.isSelected()) {
						vectors[0] = null;
					} else {
				...
				Data.YaoMaiMap.put("GouXuanValue"+setNo, tbs);
				...
				Data.YaoMaiMap.put("YaoMaiSet"+setNo, s);
			}
		});
	...
	void setP2(JPanel pane){
		...
		String[] setValue = (String[])Data.YaoMaiMap.get("YaoMaiSet"+setNo);
		if (setValue != null) {
	...
	void setP5(JPanel pane){
		...
		vectors = (Vector[])Data.YaoMaiMap.get("TableData"+setNo);
		Vector vector99 = null;
		if (vectors!=null) {
			vector99 = vectors[0];
		}
		...
		btn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (runTable.getRowCount() < 16) {
					...
					JFrame frame = (JFrame)MainUi.map.get("frame");
					JOptionPane.showMessageDialog(frame, "添加失败，总步骤数不能超过16步！", "提示", JOptionPane.ERROR_MESSAGE);
					return;
				}
			}
		});
	...
	private void setHeader(){
		...
		if (Data.YaoMaiMap.get("GouXuanValue"+setNo)!=null) {
			bs = (boolean[][])Data.YaoMaiMap.get("GouXuanValue"+setNo);
		}

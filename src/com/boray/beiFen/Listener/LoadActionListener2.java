...
public class LoadActionListener2{
	public void show() {
		/*Object[] options = { "否", "是" };
		...
		if (yes == 1) {
			...
			try {
				path = URLDecoder.decode(path,"utf-8") + "config.bry";
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	private void analyse_systemSet(File file) throws Exception{
		...
		JComboBox box8 = (JComboBox)MainUi.map.get("zhongKonGroupBox");
		new ShowAndSaveCode().show(box8.getSelectedIndex());
	}
}

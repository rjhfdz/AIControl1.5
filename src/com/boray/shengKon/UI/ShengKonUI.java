...
public class ShengKonUI implements ActionListener {
    ...
    private JRadioButton radioButton4, radioButton5;//是否启用
    ...
    private void setRightPane3(JPanel rightPane3) {
        ...
        group.add(radioButton_hdc);
        group.add(radioButton2_hdc);
        group.add(radioButton3_hdc);
        p1.add(radioButton_hdc);
        p1.add(radioButton2_hdc);
        p1.add(radioButton3_hdc);
        ...
        for (int i = 0; i < panels.length; i++) {
            ...
            panels[i].add(boxs1_hdc[i]);
            panels[i].add(sliders_hdc[i]);
            panels[i].add(boxs2_hdc[i]);
            p2.add(panels[i]);
        }
    ...
    private void setRightPane2(JPanel rightPane2) {
        rightPane2.setPreferredSize(new Dimension(830, 620));
        FlowLayout flowLayout = new FlowLayout(FlowLayout.LEFT);
        flowLayout.setVgap(13);
        rightPane2.setLayout(flowLayout);

        //final JRadioButton radioButton,radioButton2,radioButton3;//执行方式
        boxs1 = new JComboBox[2];//灯开关
        sliders = new JSlider[2];//灯亮度值
        ...
        JPanel p1 = new JPanel();
        p1.setLayout(new FlowLayout(FlowLayout.LEFT));
        TitledBorder tb = BorderFactory.createTitledBorder(BorderFactory.createLineBorder(Color.gray), "声控环境灯", TitledBorder.LEFT, TitledBorder.TOP, new Font(Font.SERIF, Font.BOLD, 12));
        p1.setBorder(tb);
        p1.setPreferredSize(new Dimension(810, 100));

        radioButton4 = new JRadioButton("启用");
        radioButton5 = new JRadioButton("不启用");
        radioButton4.setName("启用");
        radioButton5.setName("不启用");
        ...
        ActionListener listener2 = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (map != null) {
                    if (e.getActionCommand().equals("启用")) {
                        map.put("4", true);
                    } else {
                        map.put("4", false);
                    }
...
//        p1.add(field);
        ...
        final JComboBox[] boxs2 = new JComboBox[2];
        for (int i = 0; i < boxs2.length; i++) {
            ...
            //boxs1[i].addItem("关");
            boxs1[i].addItem("保持");
//            boxs1[i].setPreferredSize(new Dimension(120, 40));
            ...
            sliders[i].setPreferredSize(new Dimension(140, 80));

            boxs2[i] = new JComboBox();
//            boxs2[i].setPreferredSize(new Dimension(120, 40));
        ...
        for (int i = 0; i < panels.length; i++) {
            panels[i] = new JPanel();
            //panels[i].setBorder(new LineBorder(Color.black));
            panels[i].setPreferredSize(new Dimension(130, 300));
            panels[i].setLayout(new FlowLayout(FlowLayout.CENTER));
            panels[i].add(labels[i]);
...
//            String s = (String) map.get("3");
            ...
            bytes[4] = Byte.valueOf(MyData.ShengKonHuanJingModel);
            bytes[5] = (byte) box.getSelectedIndex();
//            if ("0".equals(s)) {
//                bytes[11] = (byte) 1;
//            } else if ("1".equals(s)) {
//                bytes[11] = (byte) 2;
...
//            }
//            int time = tp[0];
//            bytes[14] = (byte) (time / 256);
//            bytes[15] = (byte) (time % 256);
            for (int i = 0; i < 2; i++) {
                if (tp[i + 1] == 0) {
                    bytes[8 + i] = (byte) tp[i + 9];
                } else {
                    bytes[8 + i] = (byte) 0xAC;
                }
            }
            bytes[23] = ZhiLingJi.getJiaoYan(bytes);
